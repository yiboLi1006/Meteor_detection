...
class Config:
    def __init__(self):
        ...
        self.batch_size = 16  # 直接设置合适的批次大小
        ...
        self.early_stopping_patience = 8  # 早停策略
        self.lr = 0.0002  # 学习率
...
class OptimizedMeteorCNN(nn.Module):
    def __init__(self):
        ...
        # Initial conventional convolution layer
        self.conv1 = nn.Conv2d(1, 64, kernel_size=11, stride=2, padding=5)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu1 = nn.ReLU(inplace=True)
        self.pool1 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)

        # Direction-aware convolution layer for detecting meteor trails
        ...
        self.directional_bn = nn.BatchNorm2d(64)
        self.directional_relu = nn.ReLU(inplace=True)

        # Remaining layers保持与原网络相同
    ...
    def _init_directional_kernels(self):
        ...
        kernel_0 = kernel_0.reshape(1, 1, kernel_size, kernel_size)

        # 45° direction
        kernel_45 = np.exp(-(x ** 2 + y ** 2) / (2 * sigma ** 2)) * np.cos(2 * np.pi * (x + y) / (3 * np.sqrt(2)))
        kernel_45 = kernel_45.reshape(1, 1, kernel_size, kernel_size)

        # 90° direction
        kernel_90 = np.exp(-(x ** 2 + y ** 2) / (2 * sigma ** 2)) * np.cos(2 * np.pi * y / 3)
        kernel_90 = kernel_90.reshape(1, 1, kernel_size, kernel_size)

        # 135° direction
        kernel_135 = np.exp(-(x ** 2 + y ** 2) / (2 * sigma ** 2)) * np.cos(2 * np.pi * (y - x) / (3 * np.sqrt(2)))
        kernel_135 = kernel_135.reshape(1, 1, kernel_size, kernel_size)

        # Repeat for all output channels and convert to tensor
        with torch.no_grad():
    ...
    def forward(self, x):
        ...
        # Remaining forward pass保持与原网络相同
        x = self.conv2(x)
        x = self.bn2(x)
        x = self.relu2(x)
        x = self.pool2(x)
